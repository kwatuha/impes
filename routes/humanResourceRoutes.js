...
    FROM kemri_staff s
    LEFT JOIN kemri_departments d ON s.departmentId = d.departmentId
...
router.post('/export/employees-excel', auth, privilege(['employee.read_all']), async (req, res) => {
    try {
        ...
        // Fetch all employee data with joins
        const sql = `
            SELECT
                s.*,
                d.name AS department,
        ...
        // Map column headers from the frontend request
        ...
        employees.forEach(emp => {
            const rowData = {};
            // Handle manager name separately
            if (emp.managerFirstName && emp.managerLastName) {
                rowData.manager = `${emp.managerFirstName} ${emp.managerLastName}`;
            } else {
                rowData.manager = 'N/A';
            }
...
router.post('/export/employees-pdf', auth, privilege(['employee.read_all']), (req, res) => {
    try {
        const { tableHtml } = req.body;
        const doc = new PDFDocument();
        let html = htmlToPdfmake(tableHtml);

        res.setHeader('Content-Type', 'application/pdf');
        res.setHeader('Content-Disposition', 'attachment; filename=employees_report.pdf');

        doc.pipe(res);
        doc.text('Employee List', { align: 'center' });
        doc.moveDown();
        doc.addContent(html);
        doc.end();
    } catch (err) {
...
    const { 
        firstName, lastName, email, phoneNumber, departmentId, jobGroupId,
        gender, dateOfBirth, employmentStatus, startDate, emergencyContactName,
        emergencyContactRelationship, emergencyContactPhone, nationality, 
        maritalStatus, employmentType, managerId, role, placeOfBirth, 
        bloodType, religion, nationalId, kraPin, userId
    } = req.body;
    
    const sql = `
...
            firstName, lastName, email, phoneNumber, departmentId, jobGroupId, 
            gender, dateOfBirth, employmentStatus, startDate, emergencyContactName, 
            emergencyContactRelationship, emergencyContactPhone, nationality, maritalStatus, 
            employmentType, managerId, role, placeOfBirth, bloodType, religion, 
...
            firstName, lastName, email, phoneNumber, departmentId, jobGroupId, 
            gender, formatDate(dateOfBirth), employmentStatus, formatDate(startDate), 
            emergencyContactName, emergencyContactRelationship, emergencyContactPhone, 
            nationality, maritalStatus, employmentType, managerId, role, 
...
            jobGroupId = ?, gender = ?, dateOfBirth = ?, employmentStatus = ?, 
            startDate = ?, emergencyContactName = ?, emergencyContactRelationship = ?, 
            emergencyContactPhone = ?, nationality = ?, maritalStatus = ?, 
            employmentType = ?, managerId = ?, role = ?, placeOfBirth = ?, 
            bloodType = ?, religion = ?, nationalId = ?, kraPin = ?, userId = ?, 
...
    const { id } = req.params;
    try {
        // UPDATED: This query now joins with departments and job groups
        const sql = `
            SELECT 
                s.*, 
...
        `;
        const [employeeRows] = await pool.query(sql, [id]);
