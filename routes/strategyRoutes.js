...
const storage = multer.diskStorage({
    destination: (req, file, cb) => {
        ...
        // Make sure this directory exists in your backend project.
        // For example, create a folder named 'uploads' in your backend root.
        cb(null, 'uploads/');
    },
...
// Renamed from kemri_planpregistry

/**
 * @route GET /api/strategy/strategic_plans
 * @description Get all strategic plans from kemri_strategicPlans table.
 */
...
/**
 * @route GET /api/strategy/strategic_plans/:id
 * @description Get a single strategic plan by ID.
 */
router.get('/strategic_plans/:id', async (req, res) => {
    const { id } = req.params; // 'id' is camelCase from URL
    try {
        const [rows] = await pool.query('SELECT * FROM kemri_strategicPlans WHERE id = ?', [id]);
        if (rows.length > 0) {
            res.status(200).json(rows[0]); // Direct return
        } else {
...
/**
 * @route POST /api/strategy/strategic_plans
 * @description Create a new strategic plan.
 */
router.post('/strategic_plans', async (req, res) => {
    const clientData = req.body; // Contains camelCase: cidpid, cidpName, startDate, endDate, etc.

    const newPlan = {
        // 'id' is AUTO_INCREMENT, so remove if client provides it
        startDate: formatToMySQLDateTime(clientData.startDate), // Format datetime
        endDate: formatToMySQLDateTime(clientData.endDate),     // Format datetime
        // Assuming createdAt/updatedAt are auto-managed by MySQL's DEFAULT CURRENT_TIMESTAMP
        ...clientData // This brings in other camelCase fields
    };
    delete newPlan.id; // Ensure auto-increment ID is not sent by client
    ...
    try {
        ...
        if (result.insertId) {
            newPlan.id = result.insertId; // Capture the auto-generated ID
        }
...
/**
 * @route PUT /api/strategy/strategic_plans/:id
 * @description Update an existing strategic plan.
 */
router.put('/strategic_plans/:id', async (req, res) => {
    ...
    const updatedFields = {
        // Format datetime fields if present in clientData
        startDate: clientData.startDate ? formatToMySQLDateTime(clientData.startDate) : clientData.startDate,
        endDate: clientData.endDate ? formatToMySQLDateTime(clientData.endDate) : clientData.endDate,
        // Assuming updatedAt is auto-managed by MySQL's ON UPDATE CURRENT_TIMESTAMP
        ...clientData
    };
    delete updatedFields.id; // Prevent updating primary key
...
/**
 * @route DELETE /api/strategy/strategic_plans/:id
 * @description Delete a strategic plan.
 */
router.delete('/strategic_plans/:id', async (req, res) => {
    const { id } = req.params;
    try {
        console.log('Deleting Strategic Plan:', id);
        const [result] = await pool.query('DELETE FROM kemri_strategicPlans WHERE id = ?', [id]);
        if (result.affectedRows > 0) {
            res.status(204).send();
        } else {
            res.status(404).json({ message: 'Strategic plan not found' });
        ...
        res.status(500).json({ message: 'Error deleting strategic plan', error: error.message });
    }
});
...
router.get('/programs', async (req, res) => {
    try {
        const [rows] = await pool.query('SELECT * FROM kemri_programs');
...
router.get('/programs/by-plan/:planId', async (req, res) => {
    ...
    try {
        const [rows] = await pool.query('SELECT * FROM kemri_programs WHERE cidpid = ?', [planId]);
        res.status(200).json(rows);
    } catch (error) {
...
router.delete('/programs/:id', async (req, res) => {
    ...
    try {
        ...
        const [result] = await pool.query('DELETE FROM kemri_programs WHERE programId = ?', [id]);
        if (result.affectedRows > 0) {
            res.status(204).send();
        } else {
            res.status(404).json({ message: 'Program not found' });
...
router.get('/subprograms', async (req, res) => {
    try {
        const [rows] = await pool.query('SELECT * FROM kemri_subprograms');
        res.status(200).json(rows);
    } catch (error) {
...
router.get('/subprograms/by-program/:programId', async (req, res) => {
    ...
    try {
        const [rows] = await pool.query('SELECT * FROM kemri_subprograms WHERE programId = ?', [programId]);
        res.status(200).json(rows);
    } catch (error) {
...
router.delete('/subprograms/:id', async (req, res) => {
    const { id } = req.params;
    try {
        console.log('Deleting Subprogram:', id);
        const [result] = await pool.query('DELETE FROM kemri_subprograms WHERE subProgramId = ?', [id]);
        if (result.affectedRows > 0) {
            res.status(204).send();
        } else {
            res.status(404).json({ message: 'Subprogram not found' });
        ...
        res.status(500).json({ message: 'Error deleting subprogram', error: error.message });
    }
});
...
router.get('/attachments', async (req, res) => {
    try {
        const [rows] = await pool.query('SELECT * FROM kemri_planningDocuments');
        res.status(200).json(rows);
    } catch (error) {
...
router.get('/attachments/by-entity/:entityType/:entityId', async (req, res) => {
    ...
    try {
        const [rows] = await pool.query(
            'SELECT * FROM kemri_planningDocuments WHERE entityType = ? AND entityId = ?',
            [entityType, entityId]
        );
        res.status(200).json(rows);
    } catch (error) {
...
router.get('/attachments/:id', async (req, res) => {
    const { id } = req.params;
    try {
        const [rows] = await pool.query('SELECT * FROM kemri_planningDocuments WHERE attachmentId = ?', [id]);
        if (rows.length > 0) {
            res.status(200).json(rows[0]);
        } else {
...
router.delete('/attachments/:id', async (req, res) => {
    ...
    try {
        ...
        const [result] = await pool.query('DELETE FROM kemri_planningDocuments WHERE attachmentId = ?', [id]);
        if (result.affectedRows > 0) {
        ...
        console.error('Error deleting strategic planning document:', error);
        res.status(500).json({ message: 'Error deleting strategic planning document', error: error.message });
    }
});
...
router.get('/programs/:programId/export-pdf', async (req, res) => {
    ...
    try {
        ...
        // Fixed addMultiLineText function with proper scope and parameters
        ...
        if (subprograms && subprograms.length > 0) {
            subprograms.forEach((s, index) => {
                // Check if we need a new page for each subprogram
