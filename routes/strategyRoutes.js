...
router.post('/strategic_plans', async (req, res) => {
    ...
    const newPlan = {
        ...
        // Adding other fields
        ...clientData,
        // Ensure voided is set to 0 on creation
        voided: 0,
        createdAt: new Date(),
        updatedAt: new Date(),
    };
...
router.put('/strategic_plans/:id', async (req, res) => {
    ...
    const updatedFields = {
        ...
        // This will overwrite the formatted dates if present in clientData
        ...clientData,
        updatedAt: new Date(),
    };
    // Corrected to prevent updating primary key and voided column from client data
 ...
 * @description Soft delete a strategic plan.
 */
                    ...
                    if (existingSubprograms.length > 0) {
                        ...
                        await connection.query(
                            `INSERT INTO kemri_subprograms SET programId = ?, subProgramme = ?, keyOutcome = ?, kpi = ?, baseline = ?,
                             yr1Targets = ?, yr2Targets = ?, yr3Targets = ?, yr4Targets = ?, yr5Targets = ?,
                             yr1Budget = ?, yr2Budget = ?, yr3Budget = ?, yr3Budget = ?, yr4Budget = ?, yr5Budget = ?,
                             totalBudget = ?, remarks = ?`,
                            [
        ...
        for (const row of dataToImport) {
            ...
            try {
                ...
                if (existingPrograms.length > 0) {
                    ...
                    const [insertResult] = await connection.query(
                        'INSERT INTO kemri_programs SET cidpid = ?, programme = ?, departmentId = ?, sectionId = ?, needsPriorities = ?, strategies = ?, objectives = ?, outcomes = ?, remarks = ?, voided = 0',
                        [
                            planCidpId, programName, departmentId, sectionId,
                            processedRow['Program_NeedsPriorities'], processedRow['Program_Strategies'],
                            processedRow['Program_Objectives'], processedRow['Program_Outcomes'], processedRow['Program_Remarks']
