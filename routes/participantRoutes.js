...
// --- CRUD Operations for Study Participants (kemri_studyparticipants) ---

/**
 * @route POST /api/participants/study_participants/filtered
 * @description Get filtered study participants from the kemri_studyparticipants table.
...
router.post('/study_participants/filtered', async (req, res) => {
    ...
    const dataQuery = `SELECT * FROM kemri_studyparticipants ${whereClause} ORDER BY ${safeOrderBy} ${safeOrder} LIMIT 10000`; // Hardcoded limit
    ...
    const countQuery = `SELECT COUNT(*) as totalCount FROM kemri_studyparticipants ${whereClause}`;
...
router.get('/study_participants/:individualId', async (req, res) => {
    ...
    try {
        const [rows] = await pool.query('SELECT * FROM kemri_studyparticipants WHERE individualId = ?', [individualId]);
        if (rows.length > 0) {
...
router.post('/study_participants', async (req, res) => {
    ...
    try {
        const [result] = await pool.query('INSERT INTO kemri_studyparticipants SET ?', newStudyParticipant);
        if (result.insertId) {
...
router.put('/study_participants/:individualId', async (req, res) => {
    ...
    try {
        const [rows] = await pool.query('UPDATE kemri_studyparticipants SET ? WHERE individualId = ?', [fieldsToUpdate, individualId]);
        if (rows.affectedRows > 0) {
            const [updatedParticipant] = await pool.query('SELECT * FROM kemri_studyparticipants WHERE individualId = ?', [individualId]);
...
router.delete('/study_participants/:individualId', async (req, res) => {
    const { individualId } = req.params;
    try {
        const [result] = await pool.query('DELETE FROM kemri_studyparticipants WHERE individualId = ?', [individualId]);
...
router.post('/study_participants/export/excel', async (req, res) => {
    ...
    const dataQuery = `SELECT * FROM kemri_studyparticipants ${whereClause} ORDER BY ${safeOrderBy} ${safeOrder}`; // No LIMIT/OFFSET for export all
    ...
    try {
        ...
        res.setHeader('Content-Disposition', 'attachment; filename=kemri_participants_export.xlsx');
...
router.post('/study_participants/export/pdf', async (req, res) => {
    ...
    try {
        ...
        res.setHeader('Content-Disposition', 'attachment; filename=kemri_participants_report.pdf');
        res.send(pdfBuffer);
